...
29. As an operator, I want every failed Job to carry a Failure Reason (nonzero exit, timeout, runner lost, boot failure), so that I can tell what happened without trawling logs.
...
2. **Runner Channel seam** — a scripted fake runner joins the real Channel via Phoenix's channel-testing tooling using a boot token obtained from the test Provisioner. Every protocol path and failure rule is exercised here without containers: ack ⇒ running; disconnect-while-assigned ⇒ re-queued after grace; disconnect-while-running ⇒ failed with reason runner-lost; timeout ⇒ cancel pushed; token reuse rejected.
